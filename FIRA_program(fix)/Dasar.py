...
class drone():
    ...
    def set_alt(self, tujuan):
        ...
        while True:
            ...
            print("alt :", format(self.alt, '.3f'), end='\r')
    ...
    def deteksi_garis(self):
        ...
        mask_home = cv2.inRange(frame, (111,188,0), (146, 255, 255))
        contours, _ = cv2.findContours(mask, 1, cv2.CHAIN_APPROX_NONE)
        contours_home, _ = cv2.findContours(mask_home, 1, cv2.CHAIN_APPROX_NONE)
        if len(contours) > 0 :
            ...
            garis = 0
        if len(contours_home) > 0 :
            luas = [cv2.contourArea(cnt) for cnt in contours_home]
            luas.append(0)
            idx = luas.index(max(luas))
            mxluas = cv2.contourArea(contours_home[idx])
            cv2.drawContours(frame,(contours_home[idx]),-1,(0,255,0),1)
        #print(error)
        #cv2.imshow("frame",frame)
        return error, ang, mxarea, garis, mxluas
                ...
                if 3 < len(sudut) < 7 :
                    ...
                    if rasio < 2.6 :
                        ...
                        '''if len(contours_roi) > 0:
                            ...
                            #if 129 < nilai_hue < 195 and sumber == 1:
                        else:
                            vertikal = 0'''
                    else :
                        vertikal = 0
